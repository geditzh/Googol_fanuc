...
short AssemblyRobot::Home()
{
    ...
    Sleep(200);
        ...
        lastRtn = GT_SetPos(axisNumber[0], pos[0] + cHome - 3428+498);      //+498
        if (lastRtn)
            return lastRtnErr = RobotFailure;

        lastRtn = GT_SetPos(axisNumber[1], pos[1] + cHome - 355+378);        //+378
        if (lastRtn)
            return lastRtnErr = RobotFailure;

        lastRtn = GT_SetPos(axisNumber[2], pos[2] + cHome - 1794-684);      //-684
        if (lastRtn)
            return lastRtnErr = RobotFailure;
        Sleep(100);
